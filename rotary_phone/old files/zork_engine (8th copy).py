...
def handle_zork_main(digit):
    ...
    # 3. Check inventory
        ...
        if "leaflet" in zork_player["inventory"]:
            print("Zork: Reading the leaflet...")
            if os.path.exists(f"{SFX_DIR}/zork/items/read_leaflet.wav"):
                play_sound(f"{SFX_DIR}/zork/items/read_leaflet.wav", blocking=False)
        else:
            print("Zork: You have nothing to read.")
            if os.path.exists(f"{SFX_DIR}/zork/responses/nothing_to_read.wav"):
                play_sound(f"{SFX_DIR}/zork/responses/nothing_to_read.wav", blocking=False)
...
def process_item_action(item_name):
    ...
    if item_name == "mailbox":
        if not zork_player["flags"]["mailbox_open"]:
            ...
            # Interaction 2: Mailbox is open, but leaflet is still inside. Take it!
        ...
        if "leaflet" not in zork_player["inventory"]:
            zork_player["inventory"].append("leaflet")
            print("Zork: Leaflet taken. Reading: WELCOME TO ZORK...")
            if os.path.exists(f"{SFX_DIR}/zork/items/read_leaflet.wav"):
                play_sound(f"{SFX_DIR}/zork/items/read_leaflet.wav", blocking=False)
        else:
            # This triggers if they dial 6 (Read) from anywhere in the world
