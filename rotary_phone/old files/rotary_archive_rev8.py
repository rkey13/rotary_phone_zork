...
def handle_completed_number(number):
    ...
    # Cast a wide net first to find candidate files
    ...
    for filepath in candidate_files:
        ...
        # 1. Filter out things that aren't audio files
        if not filename.lower().endswith(valid_extensions):
            continue
            
        # 2. Strip the extension (e.g., "112_57b41f88.wav" -> "112_57b41f88")
        name_without_ext = os.path.splitext(filename)[0]
        
        # 3. Split by the underscore to isolate the phone number prefix
        # "11_1779932618362" becomes ["11", "1779932618362"]
        # "112" becomes ["112"]
        parts = name_without_ext.split('_')
        
        # 4. Strictly check if the very first part EXACTLY matches the dialed number
        if parts[0] == number:
            playable_files.append(filepath)
    
    if playable_files:
        # Pick one at random from the strictly filtered list
